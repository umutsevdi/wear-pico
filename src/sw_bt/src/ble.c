...
enum BT_STATUS { BT_OK, BT_ERROR_UART, BT_ERROR_CYW43 };
...
static nordic_spp_le_streamer_connection_t
    nordic_spp_le_streamer_connections[MAX_NR_CONNECTIONS];
...
static void init_connections(void)
{
    ...
    for (i = 0; i < MAX_NR_CONNECTIONS; i++) {
        nordic_spp_le_streamer_connections[i].connection_handle =
...
static nordic_spp_le_streamer_connection_t*
connection_for_conn_handle(hci_con_handle_t conn_handle)
{
    int i;
    for (i = 0; i < MAX_NR_CONNECTIONS; i++) {
        if (nordic_spp_le_streamer_connections[i].connection_handle
            == conn_handle)
...
{
    connection_index++;
    if (connection_index == MAX_NR_CONNECTIONS) { connection_index = 0; }
}
...
static void test_track_sent(nordic_spp_le_streamer_connection_t* context,
                            int bytes_sent)
{
    ...
    PRINT("%c: %" PRIu32 " sent -> %u.%03u kB/s\n", , context->name,
                               ...
                               uint8_t* packet, uint16_t size)
{
    ...
    case HCI_EVENT_LE_META:
        ...
        case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
            ...
            printf(
                "LE Connection - Connection Latency: %u\n",
                hci_subevent_le_connection_complete_get_conn_latency(packet));

            // request min con interval 15 ms for iOS 11+
            PRINT("le_conn_init:Request 15 ms connection interval\n");
            gap_request_connection_parameter_update(con_handle, 12, 12, 0,
                                                    0x0048);
            break;
        case HCI_SUBEVENT_LE_CONNECTION_UPDATE_COMPLETE:
            ...
            conn_interval =
                hci_subevent_le_connection_update_complete_get_conn_interval(
                    packet);
            printf("LE Connection - Connection Param update - connection "
                               ...
                               uint8_t* packet, uint16_t size)
{
    ...
    case ATT_EVENT_CONNECTED:
        ...
        context->test_data_len =
            ATT_DEFAULT_MTU - 4;// -1 for nordic 0x01 packet type
    ...
    case ATT_EVENT_MTU_EXCHANGE_COMPLETE:
        ...
        if (!context) break;
        context->test_data_len =
            btstack_min(mtu - 3, sizeof(context->test_data));
        PRINT("%c: ATT MTU = %u => use test data of len %u\n", , context->name,
              mtu, context->test_data_len);
        break;
...
static void nordic_can_send(void* some_context)
{
    ...
    /**********************************************************************************************/
    if (!gpio_get(BUTTON))
        memcpy(context->test_data, button_pressed, strlen(button_pressed));
    else
        memcpy(context->test_data, button_released, strlen(button_released));
    ...
    nordic_spp_service_server_request_can_send_now(&context->send_request,
...
static void nordic_spp_packet_handler(uint8_t packet_type, uint16_t channel,
                                      uint8_t* packet, uint16_t size)
{
    hci_con_handle_t con_handle;
    nordic_spp_le_streamer_connection_t* context;
    ...
    case HCI_EVENT_PACKET:
        ...
        case GATTSERVICE_SUBEVENT_SPP_SERVICE_CONNECTED:
            ...
            nordic_spp_service_server_request_can_send_now(
                &context->send_request, context->connection_handle);
            break;
...
int bt_main()
{
    if (cyw43_arch_init()) return ERROR(BT_ERROR_CYW43);
