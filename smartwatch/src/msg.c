...
enum MSGFMT sw_date_to_str(const DateTime* dt, char buffer[15])
{
    ...
    if (dt->flag & DT_WC_MONTH) memset(buffer + 4, '?', 2);
    if (dt->flag & DT_WC_DAY) memset(buffer + 6, '?', 2);
    if (dt->flag & DT_WC_HOUR) memset(buffer + 8, '?', 2);
    if (dt->flag & DT_WC_MIN) memset(buffer + 10, '?', 2);
    if (dt->flag & DT_WC_SEC) memset(buffer + 12, '?', 2);
    return MSGFMT_OK;
}
...
enum MSGFMT sw_event_new(Event* event, char* str, size_t str_s)
{
    ...
    enum MSGFMT err;
    while ((c = strchr(str, '|')) != NULL && index < 5
           && oldptr < str + str_s) {
        *c = 0;
        switch (index) {
        case 0:
...
{
    int16_t wc = 0;
    if (buffer[IDX_YEAR] == '?') wc |= DT_WC_YEAR;
    if (buffer[IDX_MONTH] == '?') wc |= DT_WC_MONTH;
    if (buffer[IDX_DAY] == '?') wc |= DT_WC_DAY;
    if (buffer[IDX_HOUR] == '?') wc |= DT_WC_HOUR;
    if (buffer[IDX_MIN] == '?') wc |= DT_WC_MIN;
    if (buffer[IDX_SEC] == '?') wc |= DT_WC_SEC;
    return wc;
}
